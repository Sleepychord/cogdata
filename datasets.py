...
class ZipDataset(Dataset):
    def __init__(self, path, num_workers, worker_id, transform=None):
        self.zip = zipfile.ZipFile(path)
        # worker_info = torch.utils.data.get_worker_info()
        # if worker_info is None:
        #     self.members = [info for info in self.zip.infolist()
        #                     if info.filename[-1] != os.sep]
        # else:
        #     all_members = [info for info in self.zip.infolist()
        #                    if info.filename[-1] != os.sep]
...
class TarDataset(Dataset):
    def __init__(self, path, num_workers, worker_id, transform=None):
        ...
        #     self.members = [x for i, x in enumerate(
        #         all_members) if i % num_workers == worker_id]
        all_members = self.tar.getmembers()
        self.members = [x for i, x in enumerate(
            all_members) if i % num_workers == worker_id]
        self.transform = transform
...
class JsonlDataset(IterableDataset):
    ...
    def __next__(self):
        ...
        key_list = ["title", "content", "abstract",
                    "q_title", "q-content", "ans-content"]
        txt = ""
        for key in key_list:
            if key in next_file:
...
class StreamingRarDataset(IterableDataset):
    ...
    def __next__(self):
        ...
        if self.data_storage is None:
            raise KeyError('There is no item named %r in the archive' %
