...
def need_datasets(p):
    p.add_argument('datasets', type=str, nargs='*', help='dataset names, None means all possible datasets.')

def need_taskid(p, required=True):
    p.add_argument('--task_id', '-t', type=str, help='id of the handling task.',required=required)
...
def get_args():
    ...
    subparser.add_argument('--data_files', type=str, nargs='+', default=[], help='file names of the handling dataset.', required=True)
    subparser.add_argument('--data_format', type=str, default='', help='format of data files.', required=True)
    subparser.add_argument('--text_files', type=str, nargs='+', default=[], help='file name of the text.')
    subparser.add_argument('--text_format', type=str, default='', help='format of the text file.')
    ...
    subparser.add_argument('--description', type=str, default='', help='description of the new task.')
    subparser.add_argument('--task_type', type=str, default=None, help='type of the handling task.', required=True) # TODO post check after import customized codes
    subparser.add_argument('--saver_type', type=str, default='binary', help='saver mode.', required=True)
    subparser.add_argument('--length_per_sample', type=int, default=1089, help='data length of one sample (Bytes).')
    subparser.add_argument('--img_sizes', type=int, nargs='+', default=[256], help='sizes of a pre-tokenized image.')
    ...
    subparser.add_argument('--nproc', type=int, default=2, help='number of processes to launch.')
    subparser.add_argument('--dataloader_num_workers', type=int, default=2, help='number of processes for dataloader per computational process.')
    subparser.add_argument('--ratio', type=float, default=1, help='ratio of data to process')
    ...
    need_taskid(subparser)
    subparser.add_argument('n', type=int, default=8, help='number of split pieces for the merge result.')
