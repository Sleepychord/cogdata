...
from webdataset import DataPipeline, tarfile_to_samples
...
class ReloadMixin:
    '''Add __iter_from__() method to a DataPipeline object'''
    def __iter_from__(self, offset_url):
        '''Iterate from a specific url, skip the previous ones.'''
        assert isinstance(self.pipeline[0], SimpleShardList)
        source = self.pipeline[0].__iter_from__(offset_url)
...
class SimpleShardList(IterableDataset):
    ...
    def __iter_from__(self, offset_url=None):
        ...
        if worker_info is not None:
            ...
            # if urls is not enough, repeat the urls
            if len(self.urls) < num_workers:
                urls = urls * (num_workers // len(urls) + 1)
                urls = urls[:num_workers]
            urls = urls[worker_id::num_workers]
        if offset_url is not None: # skip until target url 
            offset = urls.index(offset_url)
            urls = urls[offset:]
        for url in urls:
            yield dict(url=url)
    ...
    def set_data_parallel(self, dp_size, dp_rank):
        ...
        # if urls is not enough, repeat the urls
        if len(self.urls) < dp_size:
            self.urls = self.urls * ((dp_size - 1) // len(self.urls) + 1)
            assert len(self.urls) >= dp_size
        # select the urls based on dp_rank
        self.urls = self.urls[dp_rank::dp_size]
        

class SimpleDistributedWebDataset(DataPipeline):
    def __init__(self, path, process_fn, seed, shuffle_buffer=1000):
        super().__init__(
            SimpleShardList(path),
            tarfile_to_samples(),
...
class MetaDistributedWebDataset(DataPipeline, ReloadMixin):
    ...
    '''
    def __init__(self, path, process_fn, *, meta_names=[], include_dirs=None):
        ...
        self.urls = deepcopy(path) # record the unsharded urls
