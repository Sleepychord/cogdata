...
@register
class IcetkVideoTextTokenizationTask(BaseTask):
    ...
    def get_transform_fn(self, transform=None):
        ...
        def transform_fn(fp, full_filename, *args, local_transform=transform):
            ...
            try:
                ...
                # simple split
                img_groups = []
                for blocki in range(min(self.max_clip_per_video, sampled_frame_num//self.out_frame_num)):
                    imgs = [Image.fromarray(sampled_frames[blocki*self.out_frame_num+framei]).convert('RGB') for framei in range(self.out_frame_num)]
                    if local_transform is not None:
        ...
        for dataset in sub_datasets:
            ...
            for batch_videos, raw_filenames in loader:
                ...
                for i, filename in enumerate(filenames):
                    code_txt = tokenizer.encode(filename)[:txt_len-1] # at least one <pad> for separation
