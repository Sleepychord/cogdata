...
@register
class IcetkImageTextTask(BaseTask):
    '''handle tokenization
    '''
        ...
        for dataset in sub_datasets:
            ...
            for batch_imgs, raw_filenames in loader:
                ...
                for i, filename in enumerate(filenames):
                    ...
                    code_txt = tokenizer.encode(txt)[:txt_len-1] # at least one <pad> for separation
    ...
    def read_text(self, txt_files, mode):
        ...
        if mode == "json":
            ...
            for k, v in txt_list:
                ftxt = v['cnShortText']
                if 'enShortText' in v:
                    ftxt += '<n>' + v['enShortText']
                tmp.append((v['uniqueKey'], ftxt))
            text_dict = dict(tmp)
        elif mode == "txt":
            txt_list = []
            for txt in txt_files:
            ...
            for v in txt_list:
                ftxt = ''
                if 'cnShortText' in v and len(v['cnShortText']) >= 1:
                    ftxt += v['cnShortText']
                if 'enShortText' in v and len(v['enShortText']) >= 1:
                    if len(ftxt) > 0:
                        ftxt += '<n>'
                    ftxt += v['enShortText']
                if len(ftxt) < 1:
                    get_logger().warning(f"{v['uniqueKey']} do not have en/cnShortText")
                    continue
                tmp.append((v['uniqueKey'], ftxt))
            text_dict = dict(tmp)
        elif mode == "tsv":
